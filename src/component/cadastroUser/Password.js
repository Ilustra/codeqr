...
export default function Password({ route, navigation }) {
  ...
  async function createUsersocial(email, displayName, uid, providerId, password, token){
    ...
    .then(async response=>{
      ...
      try {
        realm.write(() => {
          realm.create('User', {
            name: displayName ? displayName: '', email, token, _id: _id, uid: uid, provider: providerId, token
          })
        })  
      ...
      if(senha != confSenha){
        ...
        await auth().createUserWithEmailAndPassword(email, password).then(async response=>{
          ...
          user.updateProfile({
            ...
            await createUsersocial(email, name, uid, providerId, password, token)
